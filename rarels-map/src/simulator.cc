...
ll Simulator::GetOutpFastNew(ll iPatt, ll lsb, ll msb, bool isSign) const {
    // ll lsb = 0;
    // ll msb = NetMan::GetPoNum() - 1;
    #ifdef DEBUG
    assert(iPatt < nFrame);
...
ll Simulator::GetMaxErrDist(const Simulator & oth_smlt, bool isSign, bool isCheck) const {
    if (isCheck) {
        assert(IsPIOSame(oth_smlt));
        assert(GetPoNum() < 64);
    }
    ll maxErrorDistance = 0;
    for (ll i = 0; i < nFrame; ++i) {
        ll accOut = GetOutpFast(i, isSign);
        ll appOut = oth_smlt.GetOutpFast(i, isSign);
        maxErrorDistance = max(maxErrorDistance, abs(accOut - appOut));
    }
    return maxErrorDistance;
}


std::vector<ll> Simulator::GetMinMaxSumDiff(const Simulator & oth_smlt, bool isSign, bool isCheck) const {
    vector<ll> ret({numeric_limits <ll>::max(), numeric_limits <ll>::min(), 0});
    if (isCheck) {
        assert(IsPIOSame(oth_smlt));
        assert(GetPoNum() < 64);
    }
    for (ll i = 0; i < nFrame; ++i) {
        ll accOut = GetOutpFast(i, isSign);
        ll appOut = oth_smlt.GetOutpFast(i, isSign);
        ll diff = accOut - appOut;
        ret[0] = min(ret[0], diff);
        ret[1] = max(ret[1], diff);
        ret[2] += diff;
    }
    return ret;
}
