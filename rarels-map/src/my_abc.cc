...
void NetMan::CleanUpPro() {
    ...
    // // absorb inverters
    // It seems that absorbing inverters will increase the final area and delay
    // // collect inverters
    // vector<Abc_Obj_t *> vInverters;
    // for (ll i = 0; i < GetIdMaxPlus1(); ++i) {
    //     if (!IsNode(i)) 
    //         continue;
    //     // check trivial divisors
    //     if (IsInv(i)) {
    //         for (int j = 0; j < GetFanoutNum(i); ++j) {
    //             auto pFanout = GetObj(GetFanoutId(i, j));
    //             if (!IsObjPo(pFanout)) {
    //                 cout << "inverter does not drive PO " << GetObj(i) << " " << pFanout << endl;
    //                 assert(0);
    //             }
    //         }
    //     }
